...
if __name__=="__main__":
    ...
    class WGAN_GP():
        def __init__(self,generator , discriminator , Dataloader , Dataloader_val, critic_iters =5 , gamma = 10, use_cuda = True , logdir = "None" ):
          ...
          self.G_optim = optim.Adam(self.G_Net.parameters(), lr=1e-4, betas=(0.5, 0.9))
          self.D_optim = optim.Adam(self.D_Net.parameters(), lr=1e-4, betas=(0.5, 0.9))
        ...
        def validate(self,epoch):
          with torch.no_grad():
            ...
            for i,(data_16,data_64) in enumerate(self.Dataloader_val):
              ...
              if i == 30:
                  self.writer.add_figure('Visulations ',plot_image(real_cpu_16[:6],m(real_cpu_16)[:6],generated_data_64[:6].detach(),real_cpu_64[:6]),self.steps)
